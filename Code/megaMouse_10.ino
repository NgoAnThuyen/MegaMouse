...
void loop() {
  ...
  if ((sensorLeft - previousSensorLeft) >= 100) hehe = true;
  Serial.print("state "); Serial.print(state); Serial.print('\t');
  Serial.print("hehe "); Serial.print(hehe); Serial.print('\t');
  if (running)
    {
      if (!hehe)
      {
      ...
      else 
      {
        sumErrorLeft = 0;
        cnt+=1;
        if (cnt <= 8) // tuning  number 10 for other condition // timming
        {
          turn = 0;
          baseFwd = 200;
          Serial.print(" Straight");
        }
        else if (cnt <= 10) // tuning  number 15 for other condition /timming
        {
          baseFwd = 0;
          turn = -100;          
...
{
  // return from 0 to 8 corresponding to 000 -> 111, 1 means wall.
  int wallState;
  // int thresholdLeft  = (float)threshold*1.41;
  // int thresholdRight = (float)threshold*1.41;
  // if      ((sensorLeft >= thresholdLeft) && (sensorMid >= threshold) && (sensorRight >= thresholdRight))
  //   wallState = 0;
  // else if ((sensorLeft >= thresholdLeft) && (sensorMid >= threshold) && (sensorRight < thresholdRight))
  //   wallState = 1;
  // else if ((sensorLeft >= thresholdLeft) && (sensorMid < threshold) && (sensorRight >= thresholdRight))
  //   wallState = 2;    
  // else if ((sensorLeft >= thresholdLeft) && (sensorMid < threshold) && (sensorRight < thresholdRight))
